...
## 2026-04-16 22:22:04 - 修复 Dashboard 数据显示矛盾的问题

### 1. 概念澄清与字段修正
- 修复了前台概览页 (Dashboard) 上方卡片“活跃监测任务”与下方列表“最近监测任务（暂无活跃任务）”之间的数据矛盾及语义分歧。
- 业务概念中，“活跃监测任务”指的是用户在系统登记的**计划型任务 (ScheduledJob)**，而“最近采集记录”指的是具体**实际发生过的爬取过程 (Task)**。
- 更改了后端 `/dashboard/stats` 的聚合逻辑，使得“活跃监测任务”不再统计正处于 \`processing\` 状态的瞬间动作进程，而是准确反映全局被激活的自动化长期监控计划数（\`ScheduledJob(is_active=True)\`）。

### 2. 补全后台定时任务记录展示
- 在后端 \`/spider/tasks\` API 的用户隔离筛选上增加了回退条件，使得不归属特定用户、由系统后台自动创建的 Background 增量采集记录记录（\`user_id IS NULL\`）也能正常回传到系统中。配合前面逻辑，这使得它们能正确显现在仪表盘的“最近采集记录”里。

### 3. UI 标签订正
- 修正了在列表空置时的无意义提示。不再生搬硬套展示“暂无活跃任务”，并改掉了具有歧义的表格名称，明确化为“最近采集记录”及“暂无采集记录”，还原更精准的描述。

## 2026-04-16 22:26:20 - 恢复全网舆情词云图大屏显示

### 1. 后端接口接入
- 从原有基于 Flask/Jinja2 模板的后台页面源码（`views/page/page.py`）中提炼并确认了原本词云系统的数据获取流与图片生成工具。
- 为前端 `@/services/page.ts` 新接入了向 `POST /page/wordcloud` 发起异步请求的接口 `generateWordcloud()`，利用现有新后端的 `WordCloudGenerator` 进行热词提取与轮廓绘制。

### 2. 前端 Dashboard 词云部件实装
- 对 `Dashboard.tsx` 及其配套样式模组 `Dashboard.module.css` 的前端 Grid 布局网格矩阵做出了升级改造，新拓出 `wordcloudCard` 专用空间。
- 实现进件载入时（useEffect 中）静默向服务端推送自动生成要求，并在图像下载及提取期间配备呼吸感应等待态。完成后将绘制生成的词云实时展示在全网舆情概览首屏。

## 2026-04-16 22:28:12 - 舆情体系手册内嵌与样式精简

### 1. 手册资源打包
- 为了让《网络舆情应对手册》与客户端无缝整合、提升加载效率以及实现真正意义上的离线包容性，已将项目目录的 `网络舆情应对手册.md` 正文资源彻底硬拷贝转移至 `frontend/src/assets/` 静态资源目录内。

### 2. 前端硬编码内嵌与请求解耦
- 重构了 `Manual.tsx` 的读取机制，使用 Vite 的 `?raw` 特性直接作为纯字符串引入 Markdown 并在编译期构建。
- 移除了前台通过 HTTP 强依赖后端 `fetchManualContent` 提取本地文件路径的需求，剥离了 loading 等待时长，点开手册即时渲染，丝滑无缝。

### 3. 冗余文本清理
- 按需去除了顶部类似于 `Markdown 正文已按真实文档结构渲染，不再做伪解析。来源：/Volumes/external disk/...` 的所有路径、属性解释性文案，呈现给用户更干净清爽的美化长文视图。

## 2026-04-16 22:30:25 - 修复词云图空白不显示问题

### 1. 动态服务端口寻址修复
- 诊断到之前的词云挂件之所以显示为空白或由于图片破损被浏览器隐藏，是因为其引用图片 URL 时的源跨域地址硬编码为 `http://localhost:8000`。
- 因为在 Tauri 客户端底座模式运行下，系统的底层服务端口是被守护进程随机动态分配或代理的，这使得前台发往 8000 的图片拉取请求全部失效（跨域或连接拒接）。
- 引入了系统的 `resolveBackendUrl()` 统一寻址服务接口机制，对后端返回的纯粹资源后缀路径（`/static/wordcloud/xxx.png`）进行了拼接和自包含组装。
- 现在词云图像能够跨平台在各种 Tauri 容器的宿主地址下无阻碍加载渲染出来了！

## 2026-04-16 22:35:42 - 词云生成逻辑重构：按需生成替代每次重绘

### 1. 后端重构（page.py）
- 新增 `GET /page/wordcloud` 智能接口：
  - 维护一个 `wordcloud_meta.json` 元数据文件，记录"词云图生成时的数据总量"。
  - 每次请求时对比当前数据库两端（WeiboData + DouyinData）的行数之和与元数据中的快照值，如果一致且图片存在，直接返回缓存路径，跳过耗时的 jieba 分词与 PIL 绘图过程。
  - 仅当总量有增量（爬虫新增数据后）才重新提取全量文本、重新生成并覆盖 `latest_wordcloud.png` 固定文件。
- 保留 `POST /page/wordcloud` 接口作为"强制重新生成"通道，可在 Spider 页等需要手动刷新词云的场景中调用。
