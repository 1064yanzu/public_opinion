...
# 获取当前文件的父目录的父目录
word_dir = os.path.dirname(os.path.dirname(current_file_path))
img_dir = os.path.join(word_dir, 'static', 'assets', 'images') + '\wordcloud.png'
all_dir = os.path.dirname(current_file_path)
...
def get_wordcloud():
    ...
    # 使用Pandas执行SQL查询
    sql_query = "SELECT 微博内容 FROM weibo_spid er"
...
# 加载中文停用词表
stopwords = set()
with open(all_dir+"/stopwords.txt", 'r', encoding='utf-8') as f:
    for line in f.readlines():
        stopwords.add(line.strip())
def remove_stopwords(text):
    # 分词
...
def get_wordcloud_csv(csv_path):
    ...
    for row in weibo_content_list:
        ...
        filtered_contents.append(cleaned_text)
    combined_text = ' '.join(filtered_contents)
    img = Image.open(all_dir+"/xuexi.jpg")
    img = img.convert('RGBA')
    r, g, b, a = img.split()
    img = Image.merge('RGB', (r, g, b))
    img = img.resize((1000, 1000), Image.LANCZOS)
